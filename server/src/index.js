...
server.listen(port, () => {
  if (process.env.NODE_ENV === "production") {
    console.log(
      colors.cyan(
        `Type this address to any device that is connected to this network -> `
      ) + colors.green(`${address}:${port}`)
    );
  } else {
    console.log(
      colors.cyan(
        `Brackette Server is in development mode, listening in: http://localhost:${port}`
      )
    );
  }
});
