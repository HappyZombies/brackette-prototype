  ...
  io.sockets.on("connection", socket => {
      ...
      if (
        ...
        challongeClient.matches.index({
          ...
          callback: (err, data) => {
            ...
            if (_.isEmpty(data)) {
              socket.emit("brackette error", {
                message: "There are no open tournaments..."
              });
              return;
            }
            OPEN_MATCHES = _.cloneDeep(data);
            console.log(
              "Matches received, going to send them to the host device."
            );
            ...
            challongeClient.participants.index({
              ...
              callback: (err, data) => {
                ...
                for (const i in data) {
                  ALL_PLAYERS_NAME[data[i].participant.id] =
                    data[i].participant.displayName;
                }
                console.log(
                  "Players names received, sending them to the host device."
                );
    ...
    socket.on("send private match details", matchDetails => {
      ...
      if (Object.keys(OPEN_MATCHES).length !== 0) {
        ...
        io
          .to(matchDetails.socketId)
          .emit("receive match details", matchDetails.specificMatch);
      } else {
        console.log(
          "There are no open matches to send... did the tournament end? Or is there an error ? "
        );
        socket.emit("brackette error", {
          message: "There are no open matches...did the tournament end ?"
        });
      }
    });
    ...
    socket.on("disconnect", () => {
      try {
        console.log(
          "brackette user disconnected -> ",
          ALL_BRACKETTES[socket.id].name
        );
      } catch (error) {
