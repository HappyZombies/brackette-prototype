...
const EmptyCard = () => {
  return (
    <Card containerStyle={{ textAlign: 'center', height: '250px' }}><CardText style={{ padding: '90px' }}><h1>Waiting for a match...</h1></CardText></Card>
  );
};
...
class ClientView extends Component {
  ...
  render() {
    const matchDisplay = Object.keys(this.props.brackette.currentMatch).length === 0
      ? <EmptyCard />
      : <CurrentMatchDisplay currentMatch={this.props.brackette.currentMatch} socket={this.props.socket} />;
    return (
