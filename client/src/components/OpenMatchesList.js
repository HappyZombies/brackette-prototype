...
const OpenMatchesHTML = ({
  openMatches,
  brackette,
  displayAllClientsModal
}) => {
  ...
  const html = Object.keys(openMatches).map(theKey => {
    const matchTitle =
      openMatches[theKey].match.player1Name +
      " vs " +
      openMatches[theKey].match.player2Name;
    const matchRound =
      openMatches[theKey].match.round < 0
    ...
    return (
      <Paper key={theKey} style={{ height: "125px" }}>
        ...
        <div
          onClick={displayAllClientsModal.bind(null, "match_" + theKey)}
  ...
  componentDidMount() {
    ...
    socket.on("receive players names", allPlayersNames => {
      ...
      for (const key in tempBrackette.allOpenMatches) {
        tempBrackette.allOpenMatches[key].match.player1Name = _.clone(
          allPlayersNames[tempBrackette.allOpenMatches[key].match.player1Id]
        );
        tempBrackette.allOpenMatches[key].match.player2Name = _.clone(
          allPlayersNames[tempBrackette.allOpenMatches[key].match.player2Id]
        );
      }
    ...
    socket.on("matches updated", newOpenMatches => {
      ...
      for (const key in newOpenMatches) {
        newOpenMatches[key].match.player1Name =
          tempBrackette.allChallongePlayersName[
            newOpenMatches[key].match.player1Id
          ];
        newOpenMatches[key].match.player2Name =
          tempBrackette.allChallongePlayersName[
            newOpenMatches[key].match.player2Id
          ];
      }
    ...
    if (
      _.isEmpty(this.state.openMatches) &&
      !_.isEmpty(this.props.brackette.allOpenMatches)
    ) {
  ...
  render() {
    return (
      <div className="bracketteCenter">
        <ConnectedClients socket={this.props.socket} />
        <hr />
        ...
        <OpenMatchesHTML
          ...
          brackette={this.props.brackette}
          displayAllClientsModal={this.displayAllClientsModal.bind(this)}
        />
      </div>
    );
  }
